...
**Anchor links**. To quickly navigate along the same page or to link a specific section of another page. Adding an `id` to the element and using it behind the page `path` will have ready this awesome feature.
...
**Link with icon**. Any `svg` icon that is decorative or his meaning is redundant should have `aria-hidden` to no disturb the user experience.
...
<br />
<a href="/features" aria-label="New features available">
  Learn more
</a>
...
**Button with icon**. Same that links, the icon should have `aria-hidden`.
...
**Icon button**. As links if a button doesn't have text we need to add it with `aria-label` or visually hidden.
